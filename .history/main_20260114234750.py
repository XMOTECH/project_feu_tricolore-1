...
class SimulationController:
    ...
    def run(self):
        ...
        while self.is_running:
            # 1. Mise à jour du feu (Timer + Changement couleur)
            self.traffic_light.update()
            
            # 2. Mise à jour des voitures (Mouvement + Vérification feu)
            self.veh_manager.update_vehicles(self.traffic_light)
            
            # 3. Rafraîchissement écran
            self.screen.update()
            
            # Pause pour contrôler la vitesse de simulation (FPS)
            time.sleep(0.02)
